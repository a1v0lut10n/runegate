...
**Runegate** is a lightweight, high-performance Rust-based identity proxy designed to secure access to your internal or private web applications. It acts as an authentication gateway, sitting in front of your applications (like Gradio, Uvicorn, or custom dashboards) and verifying user identity before proxying requests.

Runegate operates in two primary modes:
1. **Magic-Link-Only Mode (Stateless)**: The default, lightweight mode. It authenticates users without passwords by sending them time-limited login links via email, relying purely on JWTs and in-memory/Redis state.
2. **Gateway Mode (Stateful/Durable)**: A robust, full-featured mode backed by PostgreSQL. It provides durable user identity storage, Google OAuth2 (OIDC) integration, WebAuthn/TOTP Multi-Factor Authentication, Edge Authorization tickets, and Invite-Only onboarding controls.

---

## 📖 Terminology & Concepts

If you are new to modern identity systems, here are some key acronyms used in this project:

- **OIDC (OpenID Connect)**: An identity layer built on top of the OAuth 2.0 protocol. It allows Runegate to verify a user's identity based on authentication performed by an Authorization Server (like Google). [Learn more about Google OAuth2/OIDC](https://developers.google.com/identity/openid-connect/openid-connect).
- **JWT (JSON Web Token)**: A compact, URL-safe means of representing claims to be transferred between two parties. Runegate uses JWTs to securely transmit authentication state and issue upload tickets. [Learn more at jwt.io](https://jwt.io/introduction).
- **MFA (Multi-Factor Authentication)**: A security system that requires more than one method of authentication to verify a user's identity (e.g., combining a magic link with a physical security key or authenticator app).
- **WebAuthn**: A web standard published by the W3C that allows servers to register and authenticate users using public key cryptography instead of a password (e.g., using TouchID, FaceID, or YubiKeys).
- **TOTP (Time-Based One-Time Password)**: A temporary passcode generated by an algorithm that uses the current time as a source of uniqueness. Often used in authenticator apps like Google Authenticator or Authy.
- **JWKS (JSON Web Key Set)**: A set of keys containing the public keys used to verify any JSON Web Token (JWT) issued by the authorization server and signed using the RSA or ECDSA algorithms.

---

## ✨ Features

- 📧 **Passwordless Authentication**: Magic link login via Gmail SMTP (TLS).
- 🌐 **OIDC Integration**: Support for Google OAuth2 Single Sign-On (Gateway Mode).
- 🔐 **Multi-Factor Authentication (MFA)**: Support for WebAuthn (security keys/biometrics) and TOTP (authenticator apps).
- 🗄️ **Durable Identity & Audit**: PostgreSQL-backed user identities, invite codes, and audit logs.
- 🎟️ **Edge Authorization**: Generates short-lived JWT upload tickets and hosts a public JWKS endpoint for downstream verification.
- 🎨 **Pluggable UI**: Configurable authentication UIs (`static`, `phenotyper`, or `external`).
- 🔁 **Stateless Fallback**: Option to run entirely in-memory without a database (`magic-link-only` mode).
- 🔒 **Secure-by-Default**: Single-use magic links, strict session cookies, and configurable expiry.
- ⚡ **High Performance**: Built on `actix-web` with `sqlx` and `reqwest`.
- 📊 **Observability**: Structured logging with `tracing` and JSON formatting support.
- 🛡️ **Rate Limiting**: Multi-layered, configurable rate limiting (IP-based and Email-based).
- 🪪 **Identity Propagation**: Injects upstream headers (e.g., `X-Forwarded-User`) to pass identity to the target.
- 📡 **Streaming Proxy**: Streams upstream responses for progress bars and large downloads.

---

## 🔄 Recent Changes

- **Operational Modes**: Split the application into `Gateway Mode` (PostgreSQL backed) and `Magic-Link-Only Mode` (Stateless).
- **OIDC & MFA**: Added support for Google OAuth2 login and scaffolded WebAuthn/TOTP endpoints for escalated privilege sessions.
- **Durable Identity**: Integrated `sqlx` (v0.8.6) with a robust schema for users, OAuth identities, MFA enrollments, and audit logs.
- **Invite-Only Onboarding**: Added `RUNEGATE_SIGNUP_POLICY=invite_only` to restrict new account creation, enforced via a new Admin API (`/admin/invites`).
- **Edge Authorization**: Implemented `/upload-ticket` to mint RS256 JWTs for upstream services, and `/keys/upload_jwks.json` to expose the public keys.
- **Pluggable UI**: Introduced an `AuthUiRenderer` trait, enabling different login screen aesthetics (`static` vs `phenotyper`).
- **Session Hardening**: Sessions now use a two-stage progression (`PREAUTH` to `SESSION`), and magic links utilize single-use JWT IDs (`jti`) to prevent replay attacks.
- Configurable cookie domain, session cookie name, and strict middleware ordering.
- Proxy path fix: `/proxy/*` maps directly to the target root path `/*`.
- Identity headers injection (opt-in) and streaming responses (default ON).
